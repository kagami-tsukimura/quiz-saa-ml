...
class Choice:
    def __init__(self):
        self.A = "ファイルを分析S3バケットにコピーするLambda関数を作成します。分析S3バケットのS3イベント通知を作成します。イベント通知の送信先としてLambdaおよびSageMakerパイプラインを設定します。"
        self.B = "ファイルを分析S3バケットにコピーするLambda関数を作成します。イベント通知をAmazon EventBridge (Amazon CloudWatch Events) に送信するように分析S3バケットを設定します。EventBridgeでObjectCreatedルールを構成します。ルールのターゲットとしてLambdaおよびSageMakerパイプラインを設定します。"
        self.C = "S3バケット間のS3レプリケーションを設定します。分析S3バケットのS3イベント通知を作成します。イベント通知の送信先としてLambdaおよびSageMakerパイプラインを設定します。"
        self.D = "S3バケット間のS3レプリケーションを設定します。イベント通知をAmazon EventBridge (Amazon CloudWatch Events) に送信するように分析S3バケットを設定します。EventBridgeでObjectCreatedルールを構成します。ルールのターゲットとしてLambdaおよびSageMakerパイプラインを設定します。"
...
col1.write("")
col1.markdown(
    "<h2 style='text-align: center;'>1. SageMaker</h2>", unsafe_allow_html=True
)
